...
void matrix_multi(float *X, float t, size_t m, size_t n) 
{
    for(size_t i = 0; i < m; i++) {
        for(size_t j = 0; j < n; j++) {
            X[i * n + j] *= t;
...
{
    for (size_t i = 0; i < m; i++) {
        float max_val = *std::max_element(X + i * n, X + (i + 1) * n);
        float sum = 0;
        for (size_t j = 0; j < n; j++) {
            X[i * n + j] = std::exp(X[i * n + j] - max_val);
            sum += X[i * n + j];
        }
        for (size_t j = 0; j < n; j++) {
            X[i * n + j] /= sum;
...
float* one_hot(const unsigned char *y, size_t y_start, size_t y_end, size_t k) 
{
